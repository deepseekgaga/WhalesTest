...
本阶段不实现母页随机链接提取、Excel 行推进、最终“接受”按钮、手机号码注册、商业短信平台接入或任何人机验证绕过。
...
6. 优先通过按钮所属表单 `requestSubmit(button)` 提交，否则点击发送按钮。
...
- 辅助页首次稳定后立即读取。
- 没有验证码时每 15 秒刷新辅助页。
- 在约 15、30、45 秒执行最多三次刷新。
- 60 秒边界做最后一次有界读取，仍无验证码返回 `sms_code_not_found`。
- 慢加载不能无限延长 60 秒总截止时间。
